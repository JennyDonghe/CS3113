...
void ShaderProgram::load(const char *vertex_shader_file, const char *fragment_shader_file) {
    ...
    m_use_spotlight_uniform = glGetUniformLocation(m_program_id, "use_spotlight");
...
void ShaderProgram::set_use_spotlight(bool use)
{
    glUseProgram(m_program_id);
    glUniform1i(m_use_spotlight_uniform, use ? 1 : 0);
}
